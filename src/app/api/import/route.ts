...
function expandPRNDatesToNextYear(dayNames: string[]): string[] {
  ...
  const allowedDays = new Set(
    dayNames.map((d) => DAY_NAME_TO_INDEX[d]).filter((i) => i !== undefined),
  );
  const dates: string[] = [];
...
function createDefaultRules() {
  const rules = [
    ...
    },
    {
      ...
      ruleType: "hard" as const,
      category: "staffing" as const,
      description: "Patient ratio must not exceed census band limit",
    ...
    },
    {
      ...
      description: "Staff must have minimum 10 hours rest between shifts",
    ...
    },
    {
      ...
      ruleType: "hard" as const,
      category: "rest" as const,
      description: "Staff cannot work more than 5 consecutive days",
    ...
    },
    {
      ...
      description: "Staff assigned to ICU must have competency level 2+",
      parameters: { evaluator: "icu-competency", minLevel: 2 },
      weight: 1.0,
    },
    {
      name: "Level 1 Preceptor Required",
      ruleType: "hard" as const,
      category: "skill" as const,
      description: "Level 1 staff must have Level 5 preceptor on same shift",
      parameters: { evaluator: "level1-preceptor" },
      weight: 1.0,
    },
    {
      ...
      ruleType: "hard" as const,
      category: "skill" as const,
      description: "Level 2 staff in ICU/ER must have Level 4+ supervision",
      parameters: { evaluator: "level2-supervision" },
      weight: 1.0,
    },
    {
    ...
    },
    {
      ...
      description: "PRN staff can only be scheduled on available days",
    ...
    },
    {
      ...
      ruleType: "hard" as const,
      category: "preference" as const,
      description: "Staff with approved leave cannot be scheduled",
    ...
    },
    {
      ...
      ruleType: "hard" as const,
      category: "rest" as const,
      description: "On-call limited per week and weekend per month",
      parameters: { evaluator: "on-call-limits" },
      weight: 1.0,
    },
    {
...
function importData(data: ImportResult) {
  ...
  const staffIdByFullName = new Map(
    allImportedStaff.map((s) => [`${s.firstName} ${s.lastName}`, s.id]),
  );
    ...
    if (action === "template") {
      ...
      return new NextResponse(buffer, {
        ...
        headers: {
          ...
          "Content-Disposition":
            "attachment; filename=cah-scheduler-template.xlsx",
        },
      });
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "No data found to import. Please ensure your Excel file has Staff, Units, or Holidays sheets with data.",
        },
        { status: 400 },
      );
    }
...
// GET endpoint to export current data as Excel. An empty live export is a
// dead end for first-time users with no data yet, so when there's no staff,
// serve the bundled sample workbook instead — this also unblocks the demo,
// which now resets to an empty hospital and needs something importable.
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        "Content-Type":
          "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition":
          "attachment; filename=cah-scheduler-sample-data.xlsx",
      },
    });
  }
...
function summarisePRNDates(dates: string[]): string {
  ...
  if (weekdays.every((i) => daySet.has(i)) && !daySet.has(0) && !daySet.has(6))
    return "Weekdays";
  if (weekend.every((i) => daySet.has(i)) && daySet.size === 2)
    return "Weekends";
  const abbrevs = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  return allDays
    .filter((i) => daySet.has(i))
    .map((i) => abbrevs[i])
    .join(", ");
}
...
function exportCurrentData(): ArrayBuffer {
  ...
  const preferencesMap = new Map(
    staffPreferencesData.map((p) => [p.staffId, p]),
  );
  ...
  const holidaysRows = holidaysData.map((h) => [h.name, h.date]);
  ...
  const censusBandsSheet = XLSX.utils.aoa_to_sheet([
    censusBandsHeaders,
    ...censusBandsRows,
  ]);
  ...
  const staffLeaveSheet = XLSX.utils.aoa_to_sheet([
    staffLeaveHeaders,
    ...staffLeaveRows,
  ]);
